...
void setup() {
  ...
  if (!a.exists() && !a.isDirectory()) {
    System.err.println(subfolder + "/ does not exist.");
    exit();
  }
...
String parseLabel(String label) {
  String pattern = "<[A-Za-z 0-9]*>";
    ...
    System.err.println(subpath + " is incorrectly formatted.");
    exit();
  }
...
void parseFile(String filename, Graph g, color c) {
  ...
  if (!a.exists() || a.isDirectory()) {
    System.err.println(subpath + " does not exist.");
    exit();
  }
  ...
  if (lines.length <= 1) {
    System.err.println(subpath + " is too short and/or incorrectly formatted.");
    exit();
  }
